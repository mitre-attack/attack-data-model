...
import { z } from 'zod';
import { baseStixBundleSchema, } from './schemas/sdo/stix-bundle.schema.js';
 ...
 */
export async function registerDataSource(registration) {
        ...
        if (objParseResult && objParseResult.success) {
            ...
            if (objParseResult && objParseResult.error) {
                objParseResult.error.issues.forEach((issue) => {
                    errors.push(`Error: Path - objects.${index}.${issue.path.join('.')}, Message - ${issue.message}`);
                });
            }
            else {
 ...
 */
export function loadDataModel(id) {
