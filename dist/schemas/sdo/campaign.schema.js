...
import '../../errors';
const baseCitationSchema = z.custom((value) => {
...
const multipleCitationsSchema = z.custom((value) => {
    if (typeof value !== 'string')
        return false;
...
export const xMitreFirstSeenCitationSchema = multipleCitationsSchema.describe("One or more citations for when the object was first seen, in the form '(Citation: [citation name])(Citation: [citation name])...', where each [citation name] can be found as one of the source_name values in the external_references.");
export const xMitreLastSeenCitationSchema = multipleCitationsSchema.describe("One or more citations for when the object was last seen, in the form '(Citation: [citation name])(Citation: [citation name])...', where each [citation name] can be found as one of the source_name values in the external_references.");
/////////////////////////////////////
//
// ATT&CK Campaign
//
...
export const campaignSchema = attackBaseObjectSchema
    ...
    aliases: aliasesSchema.describe("Alternative names used to identify this campaign. The first alias must match the object's name."),
    // Optional in STIX but required in ATT&CK
    ...
    const { aliases, external_references, x_mitre_first_seen_citation, x_mitre_last_seen_citation, } = schema;
    //==============================================================================
    // Validate aliases
    //==============================================================================
    ...
    const extractCitationNames = (citations) => {
    ...
    const validateCitations = (citations, path) => {
